...
class BLACKJACK():
    ...
    def __init__(self, master):
        ...
        self.trump.Shuffle()
        # プレイヤーの設定
        self.player1 = COMPUTER(0, "相手")
        self.player2 = HUMAN(1, "自分")
        # ラベルの設置
        ...
        self.draw_button.place(x = Trump.card_width * 7 / 2, y = Trump.card_height * 2, width = Trump.card_width * 7 / 4, height = 100)
        self.not_draw_button = tk.Button(text = "don't draw", command = lambda:self.player2.update_judg_draw_card(self.button_bool_var))
        self.not_draw_button.place(x = Trump.card_width * 7 * 3 / 4, y = Trump.card_height * 2, width = Trump.card_width * 7 / 4, height = 100)
    ...
    def game_player_turn(self, player):
        ...
        player.draw_list[-1].display_surface_image(Trump.card_width * (len(player.draw_list) - 1), Trump.card_height * player.id, self.app.canvas)
